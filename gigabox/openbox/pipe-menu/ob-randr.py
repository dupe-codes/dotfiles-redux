...
# The script-base taken from "https://github.com/whiteinge/ob-randr".
# Modified by @owl4ce and converted to python3 by @mochidaz.
...
Edit your "${HOME}/.config/openbox/menu.xml" file.
Add something like the following near the top:
...
You can easily add custom commands to the menu by creating the file "${HOME}/.ob-randrrc".
The syntax looks like this:

    [Notebook]

    POTRAIT:
    --output LVDS --primary --mode 1366x768 --output VGA-0 --mode 1440x900 --left-of LVDS --rotate left

    [Netbook]

    ZOOM OUT:
    --output LVDS --scale 1.3x1.3

    ZOOM IN:
    --output LVDS --panning 1280x1024

The idea is that you can create machine-specific shortcuts. For example,
with my laptop at home I frequently connect to an external widescreen display turned sideways. On my netbook,
I frequently 'zoom out' to a higher resolution in scaled-out mode or 'zoom in' to a higher resolution in panning mode.
...
def mk_position_controls(output, name, action, outputs):
    ...
    menu = etree.Element('menu', id=output+action, type=action, label=name)
    empty = True
...
def get_xml():
    ...
    actions = (
        ...
        ('auto', '--auto'),
        ('off', '--off'),
        ('reset', ' '.join(['--auto', '--rotate normal', '--scale 1x1', '--panning 0x0']))
    )

    # The following string processing is far more verbose than necessary but if the xrandr output ever changes
    # (or I simply got it wrong to begin with) this should make it easier to fix.
    for i in xrandr_lines:
        if ' current' in i:
            # Screen 0: minimum 320x200, current 1700x1440, maximum 2048x2048.
            ...
            # VGA connected 900x1440+0+0 left (normal left inverted right x axis y axis) 408mmx255mm.
            ...
            node = etree.SubElement(root, 'menu', id=output, type='output', label=' '.join([output, mode, ' '.join(extra)]))
            modes = etree.SubElement(node, 'menu', id='%s-modes' % output, type='modes', label='modes')

            etree.SubElement(node, 'separator')

            # Add a position menu, but fill in later.
            position = etree.SubElement(node, 'menu', id='%s-position' % output, type='position', label='position')
